...
//    public static final int CONTROLLER_WHEEL = 4;
    ...
    // PID Constants
    private final static double KP_MOVE_STEERING = 0.05;
    private final static double KP_TURN_STEERING = 0.03;
    private final static double KP = -0.15; //-0.15
    private final static double KI = -0.0002;
    private final static double KD = -0.10;
    ...
    private RPMEncoder m_leftEncoder;
    private RPMEncoder m_rightEncoder;
...
//    private int m_controllerMode = CONTROLLER_XBOX;
    ...
    public Chassis() {
        ...
        try {
            m_drive = new RobotDrive(
                    new Talon(RobotMap.DRIVE_LEFT_FRONT_DSC_SLOT_ID, RobotMap.DRIVE_LEFT_FRONT_DSC_PWM_ID),
                    new Talon(RobotMap.DRIVE_LEFT_REAR_DSC_SLOT_ID, RobotMap.DRIVE_LEFT_REAR_DSC_PWM_ID),
                    new Talon(RobotMap.DRIVE_RIGHT_FRONT_DSC_SLOT_ID, RobotMap.DRIVE_RIGHT_FRONT_DSC_PWM_ID),
                    new Talon(RobotMap.DRIVE_RIGHT_REAR_DSC_SLOT_ID, RobotMap.DRIVE_RIGHT_REAR_DSC_PWM_ID));
            ...
            m_leftEncoder = new RPMEncoder(
                    RobotMap.LEFT_DRIVE_ENCODER_A_DSC_SLOT_ID, RobotMap.LEFT_DRIVE_ENCODER_A_DSC_DIO_ID, 
                    RobotMap.LEFT_DRIVE_ENCODER_B_DSC_SLOT_ID, RobotMap.LEFT_DRIVE_ENCODER_B_DSC_DIO_ID, true, 360, 5);
            m_rightEncoder = new RPMEncoder(
                    RobotMap.RIGHT_DRIVE_ENCODER_A_DSC_SLOT_ID, RobotMap.RIGHT_DRIVE_ENCODER_A_DSC_DIO_ID, 
                    RobotMap.RIGHT_DRIVE_ENCODER_B_DSC_SLOT_ID, RobotMap.RIGHT_DRIVE_ENCODER_B_DSC_DIO_ID, false, 360, 5);    
            m_leftEncoder.setDistancePerPulse(WHEEL_CIRCUM_IN/360);
            m_rightEncoder.setDistancePerPulse(WHEEL_CIRCUM_IN/360);
    ...
    public void startPID() {
    	m_leftEncoder.resetDistance();
    	m_rightEncoder.resetDistance();
        this.enable();
        m_enabled = true;
    ...
    private void setMoveMotorSpeed(double percentVbus) {
        ...
        limitAccel(percentVbus, m_maxSpeed, deltaTimeSeconds);
...
//        System.out.println("Time = " + System.currentTimeMillis() + ", Distance = " + returnPIDInput());
    }
    ...
    private double limitAccel(double percentVbus, double maxSpeed, double deltaTimeSeconds) {
        double limitedSpeed = 0.1 + (maxSpeed - 0.1) / 2.0 * deltaTimeSeconds;
...
//    }
    ...
    public double getLeftFPS() {
        return m_leftEncoder.getAveragedRPM() * WHEEL_CIRCUM_IN * 12 / 60;
    }

    public double getRightFPS() {
        return m_rightEncoder.getAveragedRPM() * WHEEL_CIRCUM_IN * 12 / 60;
    }
    ...
    public void driveWithJoystick() {        
        m_leftEncoder.updateMotorRPM();
        m_rightEncoder.updateMotorRPM();
...
//            SmartDashboard.putNumber("Left RPM ", m_leftEncoder.getAveragedRPM());
//            SmartDashboard.putNumber("Left FPS ", getLeftFPS());
//            SmartDashboard.putNumber("Left Raw ", m_leftEncoder.getRaw());
//            SmartDashboard.putNumber("Right RPM ", m_rightEncoder.getAveragedRPM());
//            SmartDashboard.putNumber("Right FPS ", getRightFPS());
